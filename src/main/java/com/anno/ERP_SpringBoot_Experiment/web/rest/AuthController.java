...
public interface AuthController {
        ...
        Response<AuthResponse> login(@Valid @RequestBody final UserLoginRequest body) throws MessagingException;
        ...
        Response<?> verifyAccount(
                        ...
                        @Valid @RequestBody(required = false) final AccountVerificationRequest body);

        @GetMapping("/send-reset-code/{email}")
        @ResponseStatus(HttpStatus.OK)
        Response<?> sendPasswordResetCode(@PathVariable final String email) throws MessagingException;
