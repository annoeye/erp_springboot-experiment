...
public class ShoppingCartService implements iShoppingCart {
    ...
    public Response<ShoppingCartDto> add(final List<CartItemRequest> items) {
        ...
        ShoppingCart cart = helper.getOrCreateCart(user, shoppingCartRepository);
        ...
        for (CartItemRequest item : items) {
            ...
            if (quantity < -9999 || quantity > 9999) {
                throw new BusinessException(ErrorCode.INVALID_QUANTITY, "Số lượng không hợp lệ");
            }
            ...
            if (quantity == 0) {
                ...
                helper.handleDecreaseItem(cart, sku, -quantity);
            }
        }
    ...
    public Response<ShoppingCartDto> getCart() {
        ...
        log.info("User {} lấy giỏ hàng", username);
        return Response.ok(
                helper.toDto(cart),
    ...
    public Response<ShoppingCartDto> clearCart() {
        ...
        ShoppingCart savedCart = shoppingCartRepository.save(cart);
        log.info("User {} đã xóa toàn bộ giỏ hàng", username);
